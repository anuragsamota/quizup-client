...
function App() {
  const [title, setTitle] = useState("QuizUp Client Application");
  const [homeActive, setHomeActive] = useState(false);
  useEffect(() => {
    setTitle("QuizUp Client Application - Updated");
  }, []);

    useEffect(() => {
    setTitle("QuizUp Client Application - Updated2");
  }, []);
  ...
  return (
    <>
      ...
      <div className="fill-primary w-16 animate-bounce"><Logo /></div>
      <div className="fill-red-500 w-64 animate-spin"><Logo /></div>
      <div className="dock dock-md">
        <button className={homeActive ? "dock-active" : ""} onClick={()=>{
          if (homeActive) setHomeActive(false);
          else setHomeActive(true);
        }}>
